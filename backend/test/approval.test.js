...
test("approval step 用机器人发审批卡片并登记 wait", async () => {
  ...
  // 按钮 URL 携带独立 secret 与 execId/nodeId（text 里不带，因为按钮独立）
  assert.ok(payload.actionCard.btns[0].actionURL.includes("/hook/dingtalk/tok-x?secret=tok-x&execId=5&nodeId=n2"));
