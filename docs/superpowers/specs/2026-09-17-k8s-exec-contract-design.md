...
- 存量流水线（如有 shell 节点引用 eci 凭证）一期即不可执行 → 该项目未 release、测试环境自用，
  接受直接破坏；列表/详情对 eci 凭证与 shell 旧参数做「已下线」提示。
...
1. shell 与 job 是否合并为同一节点类型（推荐保留两个类型名、同一后端实现，前端表单合一）？
2. `/_/hook/job/:token` 兼容端点保留（返回 410）还是直接删除？
3. ECI 下线是一期全删（推荐，未 release 无存量）还是保留读取兼容？